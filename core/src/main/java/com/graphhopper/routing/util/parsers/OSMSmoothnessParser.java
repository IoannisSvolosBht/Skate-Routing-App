...
import com.graphhopper.routing.profiles.Smoothness;
...
import static com.graphhopper.routing.profiles.Smoothness.*;
...
public class OSMSmoothnessParser implements TagParser {
    ...
    public IntsRef handleWayTags(IntsRef edgeFlags, ReaderWay readerWay, EncodingManager.Access access, long relationFlags) {
        ...
        if (smoothness == OTHER && !Helper.isEmpty(smoothnessTag)) {
            if (smoothnessTag.equals("excellent"))
                smoothness = EXCELLENT;
            else if (smoothnessTag.equals("good"))
                smoothness = GOOD;
        }
        if (smoothness != OTHER)
